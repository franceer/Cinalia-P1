        ...
        return {
            ...
            Produits: function () {
                return require('./product').query(function (qb) {
                    ...
                    qb.join('tags_products as cp', 'products.id', '=', 'cp.product_id');
                    qb.join('tags as t', 'cp.tag_id', '=', 't.id');
                    qb.where('t.path', '~', '*.' + tagPath + '.*');
            ...
            Lieux: function () {
                return require('./location').query(function (qb) {
                    ...
                    qb.join('tags_locations as cloc', 'locations.id', '=', 'cloc.location_id');
                    qb.join('tags as t', 'cloc.tag_id', '=', 't.id');
                    qb.where('t.path', '~', '*.' + tagPath + '.*');
            ...
            Looks: function () {
                return require('./look').query(function (qb) {
                    ...
                    qb.join('tags_looks as l', 'looks.id', '=', 'l.look_id');
                    qb.join('tags as t', 'l.tag_id', '=', 't.id');
                    qb.where('t.path', '~', '*.' + tagPath + '.*');
                    qb.orderBy('created_at', 'desc');
            ...
            Décors: function () {
                return require('./set').query(function (qb) {
                    ...
                    qb.join('tags_sets as s', 'sets.id', '=', 's.set_id');
                    qb.join('tags as t', 's.tag_id', '=', 't.id');
                    qb.where('t.path', '~', '*.' + tagPath + '.*');
                    qb.orderBy('created_at', 'desc');
