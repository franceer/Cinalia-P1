...
define(['jquery', 'velocity', 'app/pickedin-player'], function (jquery, velocity, PickedInPlayer) {
    ...
    TimelineBar.prototype = function () {
        ...
        var setAssetMouseHandlers = function () {
            this.$nodeElement.find('.asset-img').mouseenter(function (e) {
                ...
                $(this).children('.asset-info').velocity('stop').velocity({ opacity: 1, width: 250 }, { delay: 100, duration: 100, display: 'flex' }).children().velocity('stop').velocity({ opacity: 1 }, { delay: 200 });

            }).mouseleave(function (e) {
                $(this).children('.asset-info').velocity("stop").velocity({ opacity: 0, width: 0 }, { duration: 100, display: 'none' }).children().velocity('stop').velocity({ opacity: 0 });
            ...
            this.$nodeElement.parent().siblings('button[data-action]').on('click', function (e) {
                ...
                switch (action) {
                    ...
                    case 'start':
                        ...
                        break;
                    case 'sync':
                        var calc = prevAssets($('[data-current]'), t.$assets, t.assetNumber - 1).css('order') - 1;
